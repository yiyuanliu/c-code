...
size_t countBits(unsigned value) { return __builtin_popcount(value); }

#elif VERSION == 3
// similar to version 2
// think about three bit num abc, abc = 4a+2b+c, but we need a+b+c
// abc>>1 = 0ab = 2a+b, abc>>2 = a, abc-(abc>>1)-(abc>>2) =
// (4a+2b+c)-(2a+b)-a=a+b+c but we have 32bit, can add 0 at left, 33=11*3, when
// shift, the most left bit in 3 bits should be cleared sum by add and moudlar,
// this is hard to understand. how to expand this program to 64 bit? 4 bit,
// module 1024, because 63<64
size_t countBits(unsigned value) {
