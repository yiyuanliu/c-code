...
static void init() {
  ...
  if (!ib_ctx) {
    std::cerr << "Failed to open device " << ibv_get_device_name(dev_list[0])
              << std::endl;
    abort();
  }
  ...
  int ret = ibv_query_device_ex(ib_ctx, nullptr, &attr);
  if (ret != 0) {
    std::cerr << "Failed to quert device " << ret << "." << std::endl;
    abort();
  }

  std::cout << "Device odp cap: " << std::hex << attr.odp_caps.general_caps
            << std::dec << std::endl;
