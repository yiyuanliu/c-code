...
static void 
keep_heap_at(struct heap *hp, int i) {
...
static bool check_heap(struct heap *hp, int i) {
    ...
    bool ans =  (left >= heap_size || (hp->array[left] >= val_i && check_heap(hp, left))) &&
            (right >= heap_size || (hp->array[right] >= val_i && check_heap(hp, right)));
...
static void 
change(struct heap *hp, int i, int val) {
